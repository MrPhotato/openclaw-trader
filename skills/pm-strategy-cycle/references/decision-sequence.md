...
7.5. **提交前自检（不要跳过——历史上 Rev458/459 多次因为这一步缺失被 gate 连续打回）**

在你把 JSON 发到 `submit/strategy` 之前，逐条核对以下 4 件事，**每条都必须能在你即将发送的 JSON 里直接指给我看**：

1. **散文 ⇄ 订阅一一对应**：把 `flip_triggers` 字符串通读一遍，找出每一处 `> / < / >= / <=` + 数字的硬阈值（包括"BTC > 79K"、"Brent <$100"、"ETH <2280" 这类——逗号、`$`、`K` 都不影响 gate 计数）。每一处都必须在 `price_rechecks[]` 里有一条订阅与之对应。如果你写了 5 条阈值但只有 3 条订阅，gate 必拒 `unmonitored_flip_thresholds`。
2. **订阅字段名 + 字面量正确**：每条 `price_rechecks[]` 条目的 5 个字段名都要精确——`subscription_id` / `metric` / `operator` / `threshold` / `scope` / `reason`。**`operator` 只能是 `">="` / `"<="` / `">"` / `"<"`**——写 `"gt"`、`"lt"`、`"ge"`、`"le"`、`"greater_than"` 都会被 Pydantic 拒为 schema 错误。
3. **band 上限 ⇄ tier 一致**：如果 `target_gross_exposure_band_pct[1] > 15`，必须 `band_confidence_tier == "high"` 且 `band_confidence_evidence` ≥ 30 字；如果只到 ≤ 15，可以省略这两个字段（默认 `standard`）。同时检查 `decision_context.band_tier_eligibility.high_eligible` 是不是 `true`——`false` 时升 high 必拒。
4. **触发条件 ⇄ change_summary 一致**：如果你写"等 X 后决定加码"，`flip_triggers` 里必须真的有 X 的硬阈值，且 `price_rechecks[]` 里有对应订阅，否则你说的"等"是不会被系统监控的，PM 自己也接不到通知。

通过这 4 条之后再继续到 step 8。
