...
description: Risk Trader execution decision workflow for openclaw-trader. Use when RT needs to evaluate the current strategy, market, and risk facts every 10 minutes or on major triggers, then emit a pure JSON execution submission.
---
...
- For official cadence and PM follow-up work, submit one `execution` decision batch through the submit bridge with the current `input_id` and `live=true`.
...
2. Follow [three-stage-funnel.md](references/three-stage-funnel.md) in order.
3. Apply [escalation-and-boundaries.md](references/escalation-and-boundaries.md).
4. Emit formal JSON using [formal-output.md](references/formal-output.md), and carry the current `input_id` back to the submit bridge.
