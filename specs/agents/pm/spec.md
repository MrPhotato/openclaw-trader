...
- runtime pack 还会包含 `risk_brake_policy`，明确系统会基于单仓峰值回撤和组合峰值回撤自动执行 `reduce/exit`，PM 负责在系统动作后重写 mandate，而不是假设自己是唯一风控入口
...
- 通过 `agent_gateway` 按 `strategy.schema.json` 校验后进入消息总线
...
- PM 不直接逐模块拉数据，也不直接碰 MQ；它只拉一次 `agent_gateway` 角色包
