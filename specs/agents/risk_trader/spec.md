...
- `policy_risk` 的硬风控边界
- PM 策略中每币 `rt_discretion_band_pct` 与 `no_new_risk` 约束
...
1. 任务资格判断
- 先看 `ExecutionContext`、`policy_risk`、当前仓位偏离、`rt_discretion_band_pct`、`no_new_risk`
- 先回答“现在能不能动、有没有必要动”
...
- RT 的执行自由度必须被 PM 给出的 `rt_discretion_band_pct` 和 `no_new_risk` 约束住
