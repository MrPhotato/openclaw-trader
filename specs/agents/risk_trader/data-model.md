...
| `size_pct_of_exposure_budget` | number? | 本轮计划处理的 exposure budget 占比，分母为 `total_equity_usd * max_leverage`；`wait` 时可为空 |
...
| `reference_stop_loss_condition` | string? | 纯文字的参考止损/失效条件，供下一次 RT cadence 续接思路，不直接生成订单 |
