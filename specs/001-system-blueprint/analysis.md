...
| FR-003 3 条跨域平面 | `architecture/01-target-architecture.md`、`architecture/05-event-bus-topology.md`、`architecture/07-logging-replay-frontend.md` | 已覆盖 |
...
| FR-009 模块接口与消息协议 | `contracts/module-ports.md`、`contracts/event-routing.md`、`contracts/event-envelope.schema.json` | 已覆盖 |
...
- “事件总线 是跨域平面，不是顶层业务模块”在目标架构、消息拓扑和契约中保持一致。
...
2. 先实现统一事件协议、状态机骨架和 事件总线，再拆 `dispatch` / `strategy` / `runtime`。
